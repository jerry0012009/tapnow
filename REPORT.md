...
| 大图处理 | 已实现页面侧和后台侧缩放压缩，目标画布现有原图均小于 4 MB |

当前目标画布的真实目标节点只有 1 张业务参考图，因此真实页面本轮只能
验收单图发送；没有为了测试向用户画布永久添加多图或超过 4 MB 的新素材。
多图路径通过实际请求组装测试验证，最多发送 4 张已准备图片。超过 4 MB
的图片会尝试缩放压缩；若浏览器无法解码或压缩后仍超过上限，面板会显示
失败原因并只保留图片元数据。

0.1.5 新增的“开发者信息”和 Console 诊断包含当前 focus、节点 ID/类型、
输入、上下文、图片尺寸与准备状态、协议、模型、实际请求端点、发送图片数
和 LLM 结果摘要；不包含 API Key 或图片 data URL。
